...
# class Mlp(nn.Module):
#     def __init__(self, mlp_dim, in_features=768, out_features=None, act_layer=nn.GELU, dropout_rate=0.):
#         super().__init__()
#         out_features = out_features or in_features
#         self.fc1 = nn.Linear(in_features, mlp_dim)
#         self.act = act_layer()
#         self.fc2 = nn.Linear(mlp_dim, out_features)
...
class MlpMoeBlock(nn.Module):
    """基于 PyTorch 的稀疏 MoE 层"""
    def __init__(self, mlp_dim: int = 768, num_experts: int = 4, group_size: int = 4,
                 dropout_rate: float = 0.0, 
                 router: Optional[Dict] = None, dtype=None, split_rngs: Union[bool, Dict[str, bool]] = False):
        super().__init__()
        ...
        # self.deterministic = not self.training  # 训练时为False，推理时为Ture
        self.router_config = router or {}

        self.dtype = dtype
        self.router = self.create_router()
        self.split_rngs = split_rngs

        # 这里使用 router_cls 来创建路由器
        # self.router = self.create_router()
        self.moe_layer = sparse_moe_spmd(
            MlpBlock,
            variable_axes={'params': 0},
            split_rngs={'params': True},
            has_aux=False
        )(mlp_dim=self.mlp_dim, dropout_rate=self.dropout_rate)  # 显式传递参数
        ...
        #     has_aux=False
        # )(mlp_dim=self.mlp_dim, dropout_rate=self.dropout_rate, dtype=self.dtype, deterministic=self.deterministic,name='Mlp')
    def create_router(self):
        ...
        return NoisyTopExpertsPerItemRouter(**router_kwargs)  # 需要提供 NoisyTopExpertsPerItemRouter 实现
        # return NoisyTopItemsPerExpertRouter(**router_kwargs)

    def create_split_rngs(self):
        """创建随机数种子控制"""
        if isinstance(self.split_rngs, bool):
            return {'params': self.split_rngs, 'dropout': self.split_rngs}
        return {key: key in self.split_rngs for key in ['params', 'dropout']}

    def forward(self, inputs):
        # print("进入MlpMoeBlock")
        # print("开始执行MlpMoeBlock")
        assert inputs.dim() == 3, f'Expected ndim = 3, but got shape {inputs.shape}'
        
        # 重新排列形状 (batch, seq_len, hidden_dim) -> (num_groups, group_size, hidden_dim)
        batch_size, seq_len, hidden_dim = inputs.shape
        num_groups = batch_size * seq_len // self.group_size
        # print(f"原来的inputs:{inputs.shape}")
        inputs = inputs.reshape(num_groups, self.group_size, hidden_dim)
        # print(f"目前的inputs:{inputs.shape}") # torch.Size([394, 4, 768])
        # 路由分配
        dispatcher, metrics = self.router(inputs)
        # print(f"测试：{metrics}")
        # print(f"dispatcher:{dispatcher}")
        #----------
        # 延迟初始化 MoE 层（仅在第一次调用时初始化）
        # 调用 MoE 层，传入 dispatcher 和输入
        outputs = self.moe_layer(dispatcher, inputs)
        # print(outputs)
        #----------
        # 还原形状 (num_groups, group_size, output_dim) -> (batch, seq_len, output_dim)
        # print(f"Moe后的形状1：{outputs.shape}")
        outputs = outputs.reshape(batch_size, seq_len, outputs.shape[-1])
        # print(f"Moe后的形状2：{outputs.shape}")
...
class old_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, mlp_dim=mlp_hidden_dim, act_layer=act_layer, dropout_rate=drop)
...
class TransReID(nn.Module):  # as backbone, build_transformer_local.base
    ...
    def __init__(self, encoder: dict,img_size=(256, 128), patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,mlp_dim: int=768,
        ...
        for block_idx in range(depth):
            ...
            if block_idx in moe_mlp_layers:
                # print("使用MOE")
                mlp = MlpMoeBlock(**moe_mlp_params)
            else:
                # print("不使用MOE")
    ...
    def forward_features(self, x, camera_id, view_id, mid_feature=0, final_depth=11, occ_fix=False):
        ...
        if self.occ_aware and self.occ_block_depth != 0:
            ...
            occ_score = occ_pred.detach().softmax(dim=-1)  # torch.Size([8, 128, 2])
            # print(f"occ_score:{occ_score.shape}") 
            attn_weight = occ_score[:, :, 0] # torch.Size([8, 128])
    ...
    def load_param(self, model_path):
        ...
        key_mapping = [
            ...
            ('.attn.', '.attn.'),
            ('attn.qkv.', 'attn.qkv.')        # 保持Attention层不变（显式声明以增加可读性）
        ]
...
#     'num_experts': 4, # 小规模数据集（<50k图像）：4-8个专家, 中大规模（50k-200k）：8-16个专家
#     'group_size': 4, # 2-8
#     'router': {
