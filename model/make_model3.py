# 对应使用vit_pytorch_soft2
# 传入Attention，以及不同专家不同程度的修复
import torch
...
from .vit_pytorch_soft2 import TransReID,vit_base_patch16_224_TransReID, vit_small_patch16_224_TransReID, \
...
class build_transformer_exp(nn.Module):  # ablation
	...
	def forward(self, x, x_ori=None, cam_label=None, view_label=None,
		...
		if self.training:
			# feature extraction
			if self.two_branched:
				features, occ_pred_occ, _,metrics_occ,attn_weight_occ = self.base(x, cam_label=cam_label, view_label=view_label,
				                                      mid_feature=mid_feature, occ_fix=self.occ_aware,is_occ=True,mode='occlusion')  # [64, 129, 768]
				
				print("遮挡的self.base完毕")
				...
				# 我认为的Repairing Head: 
				# print(f"attn_weight_occ.shape:{attn_weight_occ.shape}")
				if attn_weight_occ is None:
					print("attn_weight_occ为空")
				print("开始self.head_occ")
				score_occ, feat_occ, attn_occ = self.head_occ(features,attn_weight=attn_weight_occ,mode='occlusion')
				print("完毕self.head_occ")
				# print("训练的self.head_occ完毕")
				...
				# 这里有问题：
				features_ori, occ_pred_ori, _,metrics_ori,attn_weight_ori= self.base(x_ori, cam_label=cam_label, view_label=view_label,
				                                          mid_feature=mid_feature, occ_fix=False,is_occ=False,mode='original')  # [64, 129, 768]
				print("原始的self.base完毕")
				if attn_weight_ori is None:
					print("attn_weight_ori为空")
				# print("训练的self.base——ori完毕")
				# features, ch_attn_ori = self.channel_attn(features_ori)
				ch_attn_occ = None
				ch_attn_ori = None
				if self.inference:
					# 训练实际走的这里
					print("开始self.head_ori")
					score_ori, feat_ori, attn_ori = self.head_ori(features_ori,attn_weight=attn_weight_ori,mode='original')
			...
			return {"ori": score_ori, "occ": score_occ}, \
			       ...
			       {"ori": attn_ori, "occ": attn_occ}, {"ori": ch_attn_ori, "occ": ch_attn_occ},metrics_occ,metrics_ori
		...
		# 推理
			...
			features, occ_pred, attn,_ ,attn_weight= self.base(x, cam_label=cam_label, view_label=view_label, mid_feature=mid_feature,
			                                     occ_fix=False,is_occ=True)  # [64, 129, 768]
			if attn_weight is None:
					print("推理的attn_weight为空")
			# features, _ = self.channel_attn(features) 
			feat, _ = self.head_occ(features,attn_weight=attn_weight,mode='occlusion')
...
class build_transformer_head(nn.Module):
	...
	def forward(self, features, attn_weight=None,occ_score=None, head_suppress=None,mode='occlusion'):  # label is unused if self.cos_layer == 'no'
		'''
        if self.occ_token:
		...
		# print("进入.b1_blk")
		if attn_weight is None:
			print("头部的attn_weight为空")
		b1_feat, attn,_ = self.b1_blk(features,attn_weight2=attn_weight,mode = mode)
		# print("进入.b1_blk完毕")
