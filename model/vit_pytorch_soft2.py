# 相对于vit_pytorch_soft.py，该py程序的softmoe可接收遮挡预测的attention
import math
...
# from .soft_moe3 import SoftMoE7 as SoftMoE5
# from .soft_moe2 import SoftMoE8 as SoftMoE5
from .soft_moe2 import SoftMoE11 as SoftMoE5
from .soft_moe4 import DualPathSoftMoE2,DualPathSoftMoE3
...
class Mlp(nn.Module):
    ...
    def forward(self, x, attn_weight=None, mode=None):
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
...
class MlpBlock(Mlp):
    ...
    def forward(self, inputs: torch.Tensor,attn_weight=None, mode=None) -> torch.Tensor:
        # print(f"进入MlpBlock")
...
class MoeBlock(nn.Module): 
    ...
    def forward(self, inputs,attn_weight=None, alpha=0.1, attn_weight2=None, mode=None):
        # print("进入MoeBlock")
        # Attention Block
        x = self.norm1(inputs)
        ...
        y = self.norm2(x)
        y_output = self.mlp_block(y, attn_weight=attn_weight2, mode=mode)
...
class TransReID(nn.Module):  # as backbone, build_transformer_local.base
    ...
    def __init__(self, encoder: dict,img_size=(256, 128), patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,mlp_dim: int=768,
        ...
        self.univ_experts = 32
        self.univ_factor = 1/8
        self.add_noise = True
        ...
        for block_idx in range(depth):
            ...
            if block_idx in moe_mlp_layers:
                # print("使用MOE")
                mlp = DualPathSoftMoE3(   # DualPathSoftMoELayerWrapper   SoftMoELayerWrapper
                    dim=embed_dim,
                    ...
                    univ_experts=self.univ_experts,
                    univ_factor=self.univ_factor,
                    layer_scale=self.layer_scale,
                    num_glu_experts=self.num_glu_experts,
                    freeze_moe=self.freeze_moe,
                    multi_head=False,  # 可修改为True
                )
            else:
                # print("不使用MOE") 
    ...
    def forward_features(self, x, camera_id, view_id, mid_feature=0, final_depth=11, occ_fix=False,is_occ=False,mode='original'):
        """
            ...
            for i, blk in enumerate(self.moe_blocks[:final_depth]):    
                if occ_fix:
                    ...
                    if attn_weight is None:
                        print("传入self.moe_blocks的attn_weight为空")
                    x, attn,blk_metrics= blk(x, attn_weight=attn_weight, alpha=fix_alpha[i],attn_weight2=attn_weight,mode=mode)
                    # print(x.shape)
                    ...
                    print("原始分支")
                    x, attn,blk_metrics = blk(x,attn_weight2=attn_weight,mode=mode)
        ...
        # x, occ_pred, attn,metrics
        return x, occ_pred, attn,metrics,attn_weight

    def forward(self, x, cam_label=None, view_label=None, mid_feature=0, final_depth=11, occ_fix=False,is_occ=False,mode='original'):
        x, occ_pred, attn,metrics,attn_weight = self.forward_features(x, cam_label, view_label, mid_feature, final_depth, occ_fix,is_occ,mode=mode)
        return x, occ_pred, attn,metrics,attn_weight
