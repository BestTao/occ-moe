...
def sparse_moe_spmd(
    ...
    class SparseMoeWrapper(nn.Module):
        def __init__(self, expert_fn, **expert_kwargs):
            ...
            # 初始化多个专家实例（模拟 JAXmoe 的 vmap）
            # self.num_experts = expert_kwargs.get("num_experts", 1)
            self.num_experts = self.expert_kwargs.get("params", 4)
            print(f"传入sparse——moe的专家数：{self.num_experts}")
            print("[Debug] expert_kwargs:", expert_kwargs) 
