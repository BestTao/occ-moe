# 双分支的blocks，原图使用MLP，遮挡图经过softmoe
import math
...
class MoeDualBlock(nn.Module):
    """完全独立的双分支模块"""
    def __init__(self, mlp_clean,mlp_occ,dim=768, num_heads=12, mlp_ratio=4., 
                 qkv_bias=True, qk_scale=None, drop=0., 
                 attn_drop=0., drop_path=0., 
                 num_experts=8, capacity_factor=1.0, 
                 noise_std=0.01, compute_metrics=True):
        super().__init__()
        ...
        self.mlp_clean = mlp_clean
        # ===== 遮挡分支 =====
        self.norm1_occ =copy.deepcopy(self.norm1)
        self.attn_occ = copy.deepcopy(self.attn)
        self.drop_path_occ = DropPath(drop_path) if drop_path > 0 else nn.Identity()
        self.norm2_occ = copy.deepcopy(self.norm2)
        self.mlp_occ =mlp_occ
    def forward(self, x, attn_weight=None, alpha=0.1, is_occluded=False):
        # 根据输入选择分支
        if is_occluded:
            return self._forward_occ(x, attn_weight, alpha)
        else:
            return self._forward_clean(x, attn_weight, alpha)
    
    def _forward_clean(self, x, attn_weight, alpha):
        # 原图分支前向
        shortcut = x
        x = self.norm1(x)
        x_attn, patch_attn = self.attn(x, attn_weight, alpha)
        x = shortcut + self.drop_path(x_attn)
        
        shortcut_mlp = x
        x = self.norm2(x)
        mlp_out = self.mlp_clean(x)
        
        if isinstance(mlp_out, tuple):
            x_mlp, metrics = mlp_out
        else:
            x_mlp, metrics = mlp_out, {}
            
        x = shortcut_mlp + self.drop_path(x_mlp)
        return x, patch_attn, metrics
    
    def _forward_occ(self, x, attn_weight, alpha):
        # 遮挡分支前向
        shortcut = x
        x = self.norm1_occ(x)
        x_attn, patch_attn = self.attn_occ(x, attn_weight, alpha)
        x = shortcut + self.drop_path_occ(x_attn)
        
        shortcut_mlp = x
        x = self.norm2_occ(x)
        mlp_out = self.mlp_occ(x)
        ...
        x = shortcut_mlp + self.drop_path_occ(x_mlp)
        return x, patch_attn, metrics
...
class TransReID(nn.Module):  # as backbone, build_transformer_local.base
    ...
    def __init__(self, encoder: dict,img_size=(256, 128), patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,mlp_dim: int=768,
        ...
        self.moe_occ_blocks = nn.ModuleList() # 用于遮挡预测模块
        ...
        moe_mlp_params = {**dense_mlp_params, 
                          **(self.moe)}
        # moe_mlp_layers = self.moe.get('layers', ())
        ...
        for block_idx in range(depth):
            ...
            if block_idx in moe_mlp_layers:
                mlp_clean = SoftMoE5(
                        ...
                        capacity_factor=self.moe.get('capacity_factor', 1.4),
            ...
            self.moe_blocks.append(block)
                # self.occ_moe_blocks.append(shared_block)  # 共享同一实例
    ...
    # def check_layers(model, moe_layers=(5,7,9,11)):
    #     for i, (clean, occ) in enumerate(zip(model.moe_blocks, model.occ_moe_blocks)):
    #         is_moe_layer = i in moe_layers
    #         # 验证指定层
    #         if is_moe_layer:
    #             assert clean is not occ, f"第{i}层应独立但实际共享"
    #             assert isinstance(clean.mlp_block, SoftMoE5)
    #             assert isinstance(occ.mlp_block, SoftMoE5)
    #         # 验证非指定层
    #         else:
    #             assert clean is occ, f"第{i}层应共享但实际独立"
    #             assert isinstance(clean.mlp_block, MlpBlock)
    def forward_features(self, x, camera_id, view_id, mid_feature=0, final_depth=11, occ_fix=False,is_occ=False):
        ...
        if self.occ_aware and self.occ_block_depth != 0:
            ...
            for i, blk in enumerate(self.moe_blocks[:final_depth]):   
                ...
                if occ_fix:
                    x, attn,blk_metrics= blk(x, attn_weight, fix_alpha[i],is_occluded=is_occ)
                    if i in self.moe_mlp_layers:
                        moe_features.append(x[:, 0])
                else:
                    x, attn,blk_metrics = blk(x,is_occluded=is_occ)
                    if i in self.moe_mlp_layers:
                        moe_features.append(x[:, 0])
        ...
        return x, occ_pred, attn,moe_features
