...
from typing import Any, ClassVar, Iterable, Mapping, Optional, Tuple, Type, Union,Dict,List
...
# from .soft_moe2 import SoftMoE7 as SoftMoE5
# from .soft_moe2 import SoftMoE10 as SoftMoE5
...
from timm.layers import PatchEmbed, Mlp, DropPath, AttentionPoolLatent, RmsNorm, PatchDropout, SwiGLUPacked, \
    ...
    get_act_layer, get_norm_layer, LayerType, apply_keep_indices_nlc, RotaryEmbeddingCat, to_2tuple
from timm.models._builder import build_model_with_cfg
from timm.models._manipulate import named_apply, checkpoint_seq, adapt_input_conv
from timm.models._registry import generate_default_cfgs, register_model, register_model_deprecations
from .soft_moe4 import SoftMoELayerWrapper, DualPathSoftMoELayerWrapper, DualPathSoftMoE2,DualPathSoftMoE4
...
class MoeBlock(nn.Module): 
    ...
    def forward(self, inputs,attn_weight=None, alpha=0.1):
        # print("进入MoeBlock")
        # Attention Block
        x = self.norm1(inputs)
        ...
        if isinstance(y_output, tuple):
            # print(f"元组长度：{len(y_output)}")
            y, block_metrics, dispatch_weights, combine_weights, expert_attentions, expert_contributions = y_output
            moe_data = {
            ...
            # y, block_metrics,experts_attn = y_output
            # print(y_output)
        else:
            y = y_output
            block_metrics = {}
        ...
        return x, patch_attn,block_metrics,moe_data
        # return x, patch_attn,block_metrics,experts_attn
...
# class repair_blks(nn.Module):
#     def __init__(self, embed_dim=768, depth=1,mlp_dim: int=768,norm_layer=partial(nn.LayerNorm, eps=1e-6),
#                  num_heads=12, drop_rate=0., attn_drop_rate=0., moe: Optional[dict] = None,
#                  drop_path_rate=0., ):
#         super().__init__()
#         dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]
#         self.repair_blocks = nn.ModuleList()
...
#                           **(self.moe)}
#         moe_repair_layers = moe_mlp_params.pop('layers', ())  
#         for block_idx in range(depth):
#             # ✅ 动态实例化 MLP
#             # print(moe_mlp_layers)
#             if block_idx in moe_repair_layers:
#                 # print("使用MOE")
#                 mlp = SoftMoE5(
...
#             )
#             self.repair_blocks.append(block)
#     def forward(self,x):
#         metrics={}
#         for i,blk in enumerate(self.repair_blocks):
...
#         return  x,attn,metrics['auxiliary_loss']
...
class TransReID(nn.Module):  # as backbone, build_transformer_local.base
    ...
    def __init__(self, encoder: dict,img_size=(256, 128), patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,mlp_dim: int=768,
        ...
        self.only_phi = False
        self.moe_groups_dict = {}
        # 或从构造函数参数传入
        self.slots_per_expert = 1
        self.num_experts = self.moe['num_experts']  # 192
        self.if_dualpath = False
        self.core_experts = 32
        self.occ_experts = 32
        self.occ_factor = 1/8
        self.add_noise = True
        self.noise_mult = 0.01
        self.layer_scale = False
        self.num_glu_experts = 0
        self.freeze_moe = False
        self.compress_ratio = 1.0
        self.moe_droprate = 0.0
        ...
        self.moe_mlp_layers = moe_mlp_layers
        # print(f"调用前 num_experts 类型: {type(num_experts)}, 值: {num_experts}")
        # print(f"调用前 slots_per_expert 类型: {type(slots_per_expert)}, 值: {slots_per_expert}")
        # num_experts = num_experts[0]          # 从 (192,) 提取 192
        # slots_per_expert = slots_per_expert[0] # 从 (1,) 提取 1
        for block_idx in range(depth):
            # ✅ 动态实例化 MLP
            # print(moe_mlp_layers)
            if block_idx in moe_mlp_layers:
                # print("使用MOE")
                mlp = DualPathSoftMoELayerWrapper(   # DualPathSoftMoELayerWrapper   SoftMoELayerWrapper DualPathSoftMoE4
                    dim=embed_dim,
                    num_experts=self.num_experts,
                    slots_per_expert=self.slots_per_expert,
                    add_noise=self.add_noise,
                    noise_mult=self.noise_mult,
                    moe_droprate=self.moe_droprate,
                    compress_ratio=self.compress_ratio,
                    phi=None,
                    key_proj = True,
                    core_experts=self.core_experts,
                    occ_experts=self.occ_experts,
                    occ_factor=self.occ_factor,
                    layer_scale=self.layer_scale,
                    num_glu_experts=self.num_glu_experts,
                    freeze_moe=self.freeze_moe,
                    multi_head= True  # 可修改为True
                )
            else:
                # print("不使用MOE") 
                mlp = MlpBlock(**dense_mlp_params)
    ...
    def forward_features(self, x, camera_id, view_id, mid_feature=0, final_depth=11, occ_fix=False,is_occ=False):
        """
        ...
        if self.occ_aware and self.occ_block_depth != 0:
            ...
            for i, blk in enumerate(self.moe_blocks[:final_depth]):    
                if occ_fix:
                    ...
                    # x, attn,blk_metrics= blk(x)
                    print(i)
                    # x, attn,blk_metrics,experts_attn= blk(x, attn_weight, fix_alpha[i])
                    x, attn, blk_metrics, moe_data= blk(x, attn_weight, fix_alpha[i])
                    if moe_data is not None and isinstance(moe_data, dict):
                        all_moe_data.append({
                        'layer_index': i,
                        'expert_attentions': moe_data.get('expert_attentions'),
                        'expert_contributions': moe_data.get('expert_contributions')
                    })
                    # if i in self.moe_mlp_layers:
                    #     moe_features.append(x)
                    # print(x.shape)
                    # print(f"中间打印损失：{blk_metrics}")
                    # print(f"循环中的metrics{blk_metrics}")
                    # all_expert_attns.append(experts_attn)
                else:
                    # 推理阶段
                    # print(occ_fix)
                    # print(f"可能错误的输入形状：{x.shape}")
                    # x, attn,blk_metrics,experts_attn = blk(x)
                    x, attn,blk_metrics,moe_data = blk(x)
                    if moe_data is not None and isinstance(moe_data, dict):
                        all_moe_data.append({
                        'layer_index': i,
                        'expert_attentions': moe_data.get('expert_attentions'),
                        'expert_contributions': moe_data.get('expert_contributions'),
                        'dispatch_weights':moe_data.get('dispatch_weights')
                    })
                ...
                if isinstance(blk_metrics, dict) and blk_metrics:  # 假设 MoE 块返回指标
                    ...
                    metrics[f'encoderblock_{i}'] = blk_metrics.copy()
            summed_metrics = {
                key: sum(m[key] for m in metrics.values())
                for key in next(iter(metrics.values())).keys()
            }
            metrics.update(summed_metrics)
            # print(metrics.keys())
            # print(f"metrics:{len(metrics)}")  # 'combine_weights_similarity_mean'
            # print(metrics['combine_weights_similarity_mean'].shape)
        ...
        # return x, occ_pred, attn,metrics['auxiliary_loss'] # moe_features
        return x, occ_pred, attn,metrics,all_moe_data

    def forward(self, x, cam_label=None, view_label=None, mid_feature=0, final_depth=11, occ_fix=False,is_occ=False):
        x, occ_pred, attn,metrics,moe_data = self.forward_features(x, cam_label, view_label, mid_feature, final_depth, occ_fix,is_occ)
        return x, occ_pred, attn,metrics,moe_data
    ...
    # def load_param3(self, model_path):
    #     param_dict = torch.load(model_path, map_location='cpu')
    #     if 'model' in param_dict:
    #         param_dict = param_dict['model']
    #     if 'state_dict' in param_dict:
    ...
    #         ('base.', ''),
    #         ('blocks.', 'moe_blocks.'),
    #         ('.mlp.fc1.', '.mlp_block.fc1.'),
    #         ('.mlp.fc2.', '.mlp_block.fc2.'),
    #         ('b1.', 'b1_blk.'),  # 关键映射1：b1层前缀
    #         ('b2.', 'b2_blk.'),
    ...
    #         # 调整blk层结构 (必须在循环内部处理) ----------------------------
    #         # 处理类似 b1_blk.0 -> b1_blk 的单层结构
    #         if 'b1_blk.0' in k:
    #             k = k.replace('b1_blk.0', 'b1_blk')
    #         if 'b2_blk.0' in k:
    #             k = k.replace('b2_blk.0', 'b2_blk')
    ...
    #                 handled = False
    #                 for i in range(5):
    #                     new_k = k.replace('bottleneck', f'bottleneck_{i}')
    #                     if new_k in model_dict and model_dict[new_k].shape == v.shape:
    #                         model_dict[new_k].copy_(v)
    ...
    #     # 初始化新增分类器
    #     for i in range(5):
    #         cls_name = f'classifier_{i}'
    #         if f'{cls_name}.weight' not in updated_keys and hasattr(self, cls_name):
    #             nn.init.normal_(getattr(self, cls_name).weight, std=0.001)
    #             print(f'[Init] 初始化新增分类器: {cls_name}')
...
#     # 提取分类token和网格token
#     posemb_token, posemb_grid = posemb[:, :1], posemb[0, 1:]  # [1,1,768], [242,768]
    
#     # 计算原始网格尺寸（需根据预训练模型的实际分辨率确定）
#     # 示例：原输入分辨率为 352x176，patch_size=16x16 时：
#     H_old = 352 // 16  # 22
#     W_old = 176 // 16  # 11
#     assert H_old * W_old == posemb_grid.shape[0], "原始网格尺寸计算错误"

#     # 打印调试信息
#     print(f'Resized position embedding from {posemb.shape} to {posemb_new.shape}')
#     print(f'Original grid: {H_old}x{W_old}, Target grid: {hight}x{width}')

#     # 调整维度进行二维插值
#     posemb_grid = posemb_grid.reshape(1, H_old, W_old, -1).permute(0, 3, 1, 2)  # [1,768,22,11]
...
#     # 合并分类token
#     new_posemb = torch.cat([posemb_token, posemb_grid], dim=1)
    
#     # 验证形状匹配
#     assert new_posemb.shape == posemb_new.shape, \
#         f"形状不匹配: {new_posemb.shape} vs {posemb_new.shape}"
