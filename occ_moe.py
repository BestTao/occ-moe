...
class MoeBlock(nn.Module): 
    ...
    def forward(self, inputs,alpha=0.1):
        # print("进入MoeBlock")
        # Attention Block
        x = self.norm1(inputs)
        x_, patch_attn = self.attn(x, alpha=alpha)
...
class old_Block(nn.Module):
    ...
    def forward(self, x):
        x_, patch_attn = self.attn(self.norm1(x))
...
class TransReID(nn.Module):  # as backbone, build_transformer_local.base
    ...
    def forward_features(self, x, camera_id, view_id, final_depth=11,is_occ=False):
        """
    ...
    def forward(self, x, cam_label=None, view_label=None, final_depth=11, is_occ=False):
        x, occ_pred, attn,metrics,moe_data = self.forward_features(x, cam_label, view_label, final_depth, is_occ)
